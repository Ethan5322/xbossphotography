...
// ── Validation ────────────────────────────────────────────────────────────────
function validateInput(step: BookingStep, input: string): boolean {
...
function buildResponse(step: BookingStep, data: CollectedData): string {
  ...
  switch (step) {
    ...
    case 'package': {
      const closer = PACKAGE_CLOSERS[data.package ?? ''] ?? 'Great choice! Please review your details and accept the Terms below to complete your booking.';
...
// ── Public entry point — pure, synchronous, runs on the client ────────────────
export function processChatStep(
  step: BookingStep,
  ...
  if (step !== 'greeting' && userInput) {
    valid = validateInput(step, userInput);
    if (valid) {
  ...
  const message = valid ? buildResponse(step, updatedData) : getRetryMessage(step);
  const nextStep = valid ? getNextStep(step, updatedData) : step;

  return { message, valid, nextStep, updatedData };
}
